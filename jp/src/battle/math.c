 ...
 * those four.
 * Presentation: EmbedIntAsSjis / GetItemNameLength / ShowExpDialog, and the
 * supporter-marker pair CheckForSupporterBonus / DisplaySupporterBonus.
 *
 * Callers are the executors in battle/executors.c and the AI's scoring in battle/ai.c. */
#include "common.h"
#include "object.h"
#include "battle.h"
#include "units.h"
...
u16 gTerrainBonus[14] = {[TERRAIN_PLAINS] = 0,
                         [TERRAIN_PRAIRIE] = 500,
...
/* Item names are fixed-width SJIS rows; the name ends at either a NUL or the full-width
 * space (SJIS 8140) that pads the row out. Counts CHARACTERS, not bytes. */
u8 GetItemNameLength(u8 item) {
   u8 *p;
   s32 count;

   count = 0;
   p = gItemNamesSjis[item];
   while (1) {
      if (p[0] == 0x81 && p[1] == 0x40) {
         return count;
      }
      if (p[0] == '\0') {
         return count;
...
void ShowExpDialog(s32 exp, u8 windowId) {
   u8 numDigits;
   s32 width;
   /* 経験値<4 full-width spaces> -- the digits are embedded over the spaces at byte 6. */
   s8 *pLine1 = "\x8c\x6f\x8c\xb1\x92\x6c\x81\x40\x81\x40\x81\x40\x81\x40";
   /* を獲得。 */
   s8 *pLine2 = "\x82\xf0\x8a\x6c\x93\xbe\x81\x42";
   ...
   width = numDigits * 12 / 8 + 13;
   if (numDigits * 12 & 7) {
      width++;
   }

   EmbedIntAsSjis(exp, &pLine1[6], numDigits); // Writes to rdata?
   DrawWindow(windowId, 0, 0, width * 8, 36, 160 - width * 4, 90, 0, 0);
   DrawText(12, 11, 20, 2, 0, pLine1);
   DrawText(numDigits * 12 + 48, 11, 20, 2, 0, pLine2);
...
s16 CalculateAttackDamage(UnitStatus *attacker, UnitStatus *defender) {
   ...
   damage = power -
            (defender->defVar10000 + (s16)gTerrainBonus[OBJ_TERRAIN(defenderSprite).s.terrain]);
