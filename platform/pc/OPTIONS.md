...
| `VH_HDPACK` | auto | **HD background pack** on/off (1.6). Set automatically when a valid `hdpacks/` is auto-detected beside the executable (manifest game id must match `SLUS-00447`); toggle live in the overlay ("HD PACK", greyed with no pack) and persisted here. Enabling bumps `INTERNAL RES`→≥2 and (windowed) `WINDOW SCALE`→≥2 so the HD detail is visible; dropping either to 1 disables it. Only 8bpp backgrounds are replaced — pixel-art portraits/sprites/UI stay native. `VH_HD_PACK=<dir>` (underscore) overrides the auto-detect with an explicit `backgrounds/` folder. See [../../docs/hd-pack.md](../../docs/hd-pack.md). |
