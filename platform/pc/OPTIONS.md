...
| `VH_CAM_INVERT_X` | `0` | Invert the right-stick camera *horizontal* axis (rotate direction). `1` = inverted. Also toggleable in the in-game options overlay (START+SELECT). |
| `VH_CAM_INVERT_Y` | `0` | Invert the right-stick camera *vertical* axis (raise/lower angle). `1` = inverted. Also toggleable in the in-game options overlay. |
