...
/* gSeqData's real extent (generated by build_data_segment.py, see SsSeqOpen below) -- the
 * declared-size array below is a gap-to-next-symbol placeholder, not the true bound. */
extern unsigned char gSeqData[];
extern const unsigned int PC_GenSize_gSeqData;
...
extern void PC_SpuStopVoicesUsing(const short *pcm);
...
short SsVabTransBodyPartly(unsigned char *vabBody, unsigned int size, short vabId) {
    ...
    for (int v = 0; v <= vab->numVag && v < MAX_VAG; v++) {
        ...
        if (v > 0 && vagSize > 0 && s_vabBodyStaging[vabId]) {
            /* A corrupt/hostile disc can put a size-table entry past the staged body; the game
             * gives SsVabTransBodyPartly no bank length, so this is the only place that knows
             * both offset and total. Stop the whole bank rather than decode a heap over-read. */
            if (offset < 0 || vagSize <= 0 || offset + vagSize > s_vabBodyTotal[vabId]) {
                fprintf(stderr, "SsVabTransBodyPartly: vab %d vag %d out of bounds "
                        "(offset=%d size=%d total=%d), stopping bank decode\n",
                        (int)vabId, v, offset, vagSize, s_vabBodyTotal[vabId]);
                break;
            }
            short *pcm;
...
void SsVabClose(short vabId) {
    ...
    for (int v = 0; v < MAX_VAG; v++) {
        ...
        /* A voice can still be sustaining/releasing on this bank's PCM (SsVoKeyOff/note-off
         * only mark it releasing, they don't stop the render read) -- hard-stop it before the
         * free below or PC_SpuService renders a dangling short* on the next tick. */
        if (vab->vagPcm[v]) PC_SpuStopVoicesUsing(vab->vagPcm[v]);
        free(vab->vagPcm[v]);   /* retained decoded PCM for the software SPU */
    }
    for (int i = 0; i < MAX_VOICES; i++) {
        if (s_voices[i].vabId == vabId) { s_voices[i].active = 0; s_voices[i].ownSeq = -1; }
    }
    memset(vab, 0, sizeof(*vab));
    free(s_vabBodyStaging[vabId]);
...
static int SeqProcessEvent(Seq *s) {
    ...
    int ch = status & 0x0f;
    /* Every case below that dereferences s->pos must first prove the bytes it's about to read
     * are still inside [base, end) -- a malformed/truncated song (no FF 2F) must stop cleanly
     * at the bound instead of reading past it. */
    switch (status & 0xf0) {
    case 0x80: if (s->pos + 2 > s->end) return 0;
               { int n = *s->pos++, v = *s->pos++; (void)v; SeqNoteOff(s, ch, n); break; }
    case 0x90: if (s->pos + 2 > s->end) return 0;
               { int n = *s->pos++, v = *s->pos++;
                 if (v) SeqNoteOn(s, ch, n, v); else SeqNoteOff(s, ch, n); break; }
    case 0xa0: s->pos += 2; break;                 /* poly aftertouch (ignore)   */
    case 0xb0: if (s->pos + 2 > s->end) return 0;
               { int c = *s->pos++, v = *s->pos++; /* control change             */
                 ...
                 break; }
    case 0xc0: if (s->pos + 1 > s->end) return 0;
               s->chProg[ch] = *s->pos++; break;   /* program change             */
...
short SsSeqOpen(unsigned int *seqData, short vabId) {
    ...
    /* SEQ_MAX_BYTES is only a safety cap for a foreign blob; when this song plays out of the
     * game's own generated gSeqData, bound it to the real staged size instead -- otherwise a
     * malformed song (no FF 2F) walks 0x40000 bytes past a much smaller buffer. */
    { uintptr_t addr = (uintptr_t)d, lo = (uintptr_t)gSeqData, hi = lo + PC_GenSize_gSeqData;
      if (addr >= lo && addr < hi) s->end = gSeqData + PC_GenSize_gSeqData; }
