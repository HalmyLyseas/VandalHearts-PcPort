...
leaves it NULL, needing manual reconstruction; nothing populates it at runtime. Result: LoadEvent() gives
...
# The game tree provides the symbol map and the byte-exact executable; symbol NAMES are
# shared between the trees (JP keeps US naming at JP addresses).
...
# The JP map carries no `// size:` annotations and gap-to-next overshoots (0x24c0 for the
# 0xd0-byte gAnimSet_800f2db4), so missing sizes fall back to the US annotation for the same
# name, clamped by this region's gap-to-next. See docs/pc-port/data-segment.md.
...
# The unnamed anim-set/byte window, per region. Both span the same content: just below
# gStringTable up to just below gAnimSet_80104034.
...
def read_unnamed(A):
    ...
    ents, k = [], 0
    # The real end of each array is the first non-anim entry (below); the cap is only a runaway
    # guard and must exceed every real animIdx (sArr_80102538 is indexed at 32 by a Grog-house
    # prop). Over-reading into a contiguous following array is harmless.
    while k < 256:
        e = u32(A + k*4)
...
L = ["/* platform/pc/src/pc_evt_entities.c -- GENERATED by platform/pc/tools/gen_evt_entities.py.",
     ...
     " * entries -> sEvtAltBlob anim bytes). DO NOT hand-edit; rerun the generator. PC-port data",
