...
gStringTable[100] (0x8010102c) is defined in src/core/text.c via
...
core/text.c's byte-exact placeholder blobs (D_800151C8[888] and the D_80122FBx[8]
...
that draws from it (world-map destination names via world/map.c's
gStringTable[33]/[34], and every "#N" string-table escape in DrawText/
DrawSjisText) rendered blank.

Rather than gate the byte-exact core/text.c definition, we fix it up at runtime:
...
def main():
    ...
    lines.append("/* Runtime fixup for gStringTable (0x8010102c): the byte-exact core/text.c initializer")
    ...
    lines.append("    }")
    lines.append("    gStringTable[%d] = s_pc_empty_string; /* PC sentinel used transiently by world/dojo.c */" % COUNT)
    lines.append("}")
    lines.append("")
    open(OUT, "w").write("\n".join(lines))
