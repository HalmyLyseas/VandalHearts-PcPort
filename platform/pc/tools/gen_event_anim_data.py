...
WHY: the PC data-gen (build_data_segment.py) flags these pointer-containing globals "needs manual
review" and leaves them NULL; cutscene unit sprites then resolve animData=NULL -> gfxIdx=0 ->
...
# The JP map carries no `// size:` annotations. The anim tables are structurally shared between
# the regions (same names, same event system), so missing sizes fall back to the US map's
# annotation for the same name -- clamped by the region's own gap-to-next-symbol, a hard upper
...
L = [f"/* platform/pc/src/pc_event_anim_data.c -- GENERATED by platform/pc/tools/gen_event_anim_data.py",
     f" * ({REGION} region, from byte-exact {BASENAME}). Reconstructed cutscene/event unit ANIMATION-SET",
     ...
     " * gAnimSet_80101fc0 is intentionally NOT here -- it is already real-defined in src/maps/unpack.c.",
     f" * Raw anim bytes captured verbatim from .data window [{blo:#x},{bhi:#x}) (embedded table bytes",
     " * included, never indexed); tables rebuilt to index into it. DO NOT hand-edit; rerun the",
     " * generator. Stage-2 (PC port) data only; not in the matching build. */",
     '#include "common.h"', "",
