...
| `VH_HIRES_INSET` | `1` (on) | Advanced: at internal resolution > 1, keeps tile edges sampling the tile interior instead of the dark "crust" texel at each texture-cell border — otherwise the finer sampling paints a faint dark grid along terrain/lava/water tile seams. On by default; set `0` for the raw (gridded) look. No effect at `1`×. |
