...
Indexed by `CLASS_*` (`src/battle/math.c:175-303`). **Attack, Defense, MaxHp and Agility are all
...
`gClassMpMultiplier`: maxMp = `mult × level`, capped 99 (`battle/math.c:882`). **MAGE = 2, PRIEST = 2,
...
Used in `PopulateMovementGrid` (`src/battle/path_grids.c`): `0` = free (entered without spending movement),
...
`atkVar10000`/`defVar10000` but is **read nowhere** (`src/battle/math.c`); `unit.agility` is read once,
